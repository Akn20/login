    ...
    <tr>
        ...
        <td>
            @include('partials.status-toggle', [
                'id'      => $holiday->id,
                'url'     => route('admin.holidays.toggleStatus', $holiday->id),
                'checked' => (bool)$holiday->status,
            ])
        </td>
        <td class="text-end">
            <div class="d-flex justify-content-end gap-2">
                {{-- View Button --}}
                <a href="{{ route('admin.holidays.show', $holiday->id) }}" class="btn btn-outline-secondary btn-icon rounded-circle btn-sm">
                    <i class="feather-eye"></i>
                </a>

                {{-- Edit Button --}}
                <a href="{{ route('admin.holidays.edit', $holiday->id) }}" class="btn btn-outline-secondary btn-icon rounded-circle btn-sm">
                    <i class="feather-edit-2"></i>
                </a>

                {{-- Delete Button --}}
                <form action="{{ route('admin.holidays.delete', $holiday->id) }}" method="POST" onsubmit="return confirm('Move to trash?')">
                    @csrf
                    @method('DELETE')
