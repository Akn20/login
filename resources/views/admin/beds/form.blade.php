...
<div class="row">
    ...
    <div class="col-md-6 mb-3">
        ...
        <input type="text" name="bed_code" id="bed_code" class="form-control"
            value="{{ old('bed_code', $bed->bed_code ?? '') }}" required>
    </div>
        ...
        <select name="room_number" class="form-control" required>
            ...
            @foreach($rooms as $room)
                <option value="{{ $room->room_number }}">
                    Room {{ $room->room_number }} ({{ $room->ward->ward_name ?? '' }})
                </option>
            @endforeach
    ...
    <div class="col-md-6 mb-3">
        ...
        <select name="bed_type" class="form-control" required>
            @foreach(['General', 'ICU', 'Private', 'Semi-Private'] as $type)
                <option value="{{ $type }}" {{ old('bed_type', $bed->bed_type ?? '') == $type ? 'selected' : '' }}>
                    {{ $type }}
                </option>
            @endforeach
        </select>
    ...
    <div class="col-md-6 mb-3">
        ...
        <select name="status" class="form-control" required>
            @foreach(['Available', 'Occupied', 'Maintenance', 'Cleaning'] as $status)
                <option value="{{ $status }}" {{ old('status', $bed->status ?? 'Available') == $status ? 'selected' : '' }}>
                    {{ $status }}
                </option>
            @endforeach
        </select>
